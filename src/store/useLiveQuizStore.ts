...
export const useLiveQuizStore = create<LiveQuizState>((set, get) => ({
  ...
  connectWebSocket: (sessionId: string, username: string) => {
    ...
    const wsBaseUrl = window.location.hostname !== 'localhost' ? 'https://student-intelligence-system.onrender.com/ws' : (import.meta.env.VITE_WS_URL || 'http://localhost:8082/ws');
